...
# verify
	...
	for key in $LINUX_GPG_KEYS; do \
		...
		; do \
			if gpg  --no-tty --verbose --keyserver "$mirror" --keyserver-options timeout=5 --recv-keys "$key"; then \
				break; \
			fi; \
		done; \
...
RUN tcl-tce-load \
		...
		util-linux \
		xz \
		gcc_libs liblzma expat2 libgpg-error libgcrypt libssh2 tcp_wrappers portmap libtirpc ntpclient glib2 libffi fuse pcre udev-lib udev-extra liblvm2
